...
//session_start();
$actual_amt_485 = (double)9.95;
//echo $actual_amt_485;
$num_days_485 = (int)365;
...
session_start();
		...
		$stmt = $dbCon->prepare("SELECT UserName FROM room_mates_users_570 WHERE UserName=?");
	...
	if($UN == $dbUN){
	$msg2 = "UserName Exists Already Sorry";
	}

			 /*$UN_Match = "SELECT UserName FROM room_mates_users_570 WHERE UserName = '$UN'";
			$results = mysqli_query($dbCon,$UN_Match);
			$UN_Check = mysqli_num_rows($results);
				if($UN_Check > 0){
					$msg2 = "UserName Already Exists Sorry";
				} */
				$Email_Match = $dbCon->prepare("SELECT Email FROM room_mates_users_570 WHERE Email =?");
				
				$Email_Match ->bind_param("s",$Email);
				
				$Email_Match->execute();
				
				$match_e = $Email_Match->get_result();
				
				$matches = $match_e->fetch_assoc();
				
				$dbEmail = $matches['Email'];
				
				if($Email == $dbEmail){
				 $msg = "Email Exists Please Log In";
				}
        ...
        <div class="col-md-6">
            <div class="panel panel-default" style="margin-top:15%;">
                <div class="panel-body" style="background-color: #a6e1ec;">
                    ...
                    <form class="form-horizontal" action="Register365Days.php" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <div class="col-sm-offset-3 col-sm-3">
                                <input name="Register" style="background-color: #5bc0de; color:white; font-size:18px;" class="btn" type="submit" id="Register" value="Register">
                            </div>
                        </div>
                    </form>
                </div>
