...
//session_start();
$actual_amt_485 = (double)1.95;
//echo $actual_amt_485;
$num_days_485 = (int)7;

$paypal_token = $_GET['token'];
//echo "Token PayPal:".$paypal_token;

if($paypal_token = ' '){
	die("Token is Blank; Contact Support if this is a mistake");
}
...
session_start();
		...
		$row = $result->fetch_assoc();
			...
			//////////////////////////
			$email_query = $dbCon->prepare("SELECT UserName,Email FROM room_mates_users_570 WHERE Email =?");
		$email_query->bind_param("s",$Email);
		$email_query->execute();
		$email_res = $email_query->get_result();
		$rows = $email_res->fetch_assoc();
		$UN_Check = $rows['UserName'];
		$Email_Check = $rows['Email'];
		//////////////
		if($UN == $dbUN){
			$msg2 = "Sorry UserName Already Exists";
			if($Email == $dbEmail){
	...
	elseif($Email != $dbEmail && $UN != $dbUN && $Email_Check != $Email){
			$status = 'New';
				$Type = 'User'; 
					//encrypt password here using bcrypt		
					$encrypt_pw = password_hash($PW,PASSWORD_DEFAULT);
					$insert_rm = $dbCon->prepare("INSERT INTO room_mates_users_570(FName,LName,Password,UserName,Type,Email,Account_Status) Values (?,?,?,?,?,?,?)");

					$insert_rm->bind_param("sssssss",$FN,$LN,$encrypt_pw,$UN,$Type,$Email,$Account_Status);
					$insert_rm->execute();	 
						if ($insert_rm) {
				...
				$rows = $select_rows->fetch_assoc();
						...
						if($LHT_insert){
							...
							$questionaire = $dbCon->prepare("INSERT INTO Profile_Answers_570 (UserID,Gender) Values(?,?)");
							$questionaire->bind_param("is",$id,$Gender);
	...
	if($imgType != "jpg" && $imgType != "jpeg" && $imgType != "gif" && $imgType != "png" && $imgType != "PNG" && $imgType !="GIF" && $imgType !="JPG" && $imgType != "JPEG")
	{
        ...
        <div class="col-md-6">
            <div class="panel panel-default" style="margin-top:15%;">
                <div class="panel-body" style="background-color: #a6e1ec;">
                    ...
                    <form class="form-horizontal" action="Register1Week.php" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <div class="input-group">
                                    <span class="input-group-addon" style="background-color: #5bc0de;"><span class="glyphicon glyphicon-user"></span></span>
                                    <select name="Gender" class="text-center form-control">
									<option>Male</option>
